...
const Navigasi = ({ handle }) => {
  ...
  return (
    <ul className="w-full font-outfit">
      {menu.map((item, index) => (
        <NavLink
          to={item.path}
          key={index}
          className={({ isActive }) =>
            isActive ? "text-primary text-medium" : "text-grey-dark"
          }
        >
          <li onClick={handle} className="text-lg/loose flex justify-center items-center gap-2 group/menu">
            {item.name}
            <svg
              ...
              stroke-width="1.5"
              stroke="currentColor"
              class="w-4 h-4 transition-all group-hover/menu:translate-x-1"
            >
              <path
  ...
  return (
    <>
      <div className="relative">
        <header className="absolute w-full bg-white flex items-center justify-between px-4 py-3 select-none z-10">
          <a href="/">
            <img
              src={profile}
              alt="logo"
